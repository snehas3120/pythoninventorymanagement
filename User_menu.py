...
class User:
    ...
    def user_mainmenu(self,a,b):
        ...
        self.tableframe.place(x=1230, y=300, anchor=NE)
    ...
    def make_invoice(self):
        ...
        'Quantity', 'Price','CGST','SGST',"Total Amount",'Date','Time'), selectmode="browse", height=6, yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
        self.tree.column('#0', stretch=NO, minwidth=0, width=0)
    ...
    def user_input(self):
       self.cur.execute('select max(trans_id) from saless')
       ...
       salesperson = mycombobox(self.entryframe,width=20,textvariable=self.addstaff,font="roboto 12")
       salesperson.place(x=80,y=30,height=30)
       ...
       self.cur.execute("select name from staffs")
       li=self.cur.fetchall()
       self.staffs = []
       
       for i in range(0, len(li)):
           if (self.staffs.count(li[i][0]) == 0):
               self.staffs.append(li[i][0])
           
       salesperson.set_completion_list(self.staffs)
    ...
    def transtableadd(self):
        ...
        if (messagebox.askyesno('Alert!','Do you want to proceed?') == False):
            return
        a=[]
        self.cur.execute("select max(invoice) from saless")
        self.invoice=self.cur.fetchall()
        
        self.invoice = self.invoice+[1];
        for i in x:
            l=self.tree.item(i)
            a.append(l['values'])
        for i in a:
            s = (str(i[5])).split('-')
            i[5] = s[2] + "-" + s[1] + "-" + s[0]
            self.cur.execute("insert into saless values (?,?,?,?,?,?,?,?,?,?)",(int(i[0]),int(self.invoice),int(i[1]),int(i[3]),i[5],i[6]))
    ...
    def makeprint(self):
           ...
           x = datetime.datetime.now()
           x = str(x.hour) + ' - ' + str(x.minute) + ' - ' + str(x.second)
           dir=r"C:\Users\sneha\Desktop\practice"
           if not os.path.exists(dir):
               os.makedirs(dir)
           shop="\t\t\t\t\t Aanoj's Unisex Saloon \n"
